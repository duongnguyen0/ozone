...
public class SCMStateMachine extends BaseStateMachine {
  ...
  public CompletableFuture<Message> applyTransaction(
    ...
    try {
      ...
      transactionBuffer.updateLatestTrxInfo(TransactionInfo.valueOf(TermIndex.valueOf(trx.getLogEntryUnsafe())));
    } catch (Exception ex) {
      applyTransactionFuture.completeExceptionally(ex);
  ...
  public void notifyLeaderReady() {
    ...
    if (scm.getScmHAManager().getRatisServer().getDivision().getInfo().isLeaderReady()) {
